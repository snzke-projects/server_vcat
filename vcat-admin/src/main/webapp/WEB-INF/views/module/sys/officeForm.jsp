...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			var distributionIdStr = "${office.distributionIds}";
			$('.distribution').each(function(){
				if(distributionIdStr.indexOf(this.value) >= 0){
					this.checked = true;
				}
			});
			$("#name").focus();
...
<body>
	...
	<form:form id="inputForm" modelAttribute="office" action="${ctx}/sys/office/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">配送方授权:</label>
			<div class="controls">
				<c:forEach items="${distributionList}" var="distribution" varStatus="distributionStatus">
					<input id="distributionCheckBox_${distributionStatus.index}" type="checkbox" name="distributionIds" value="${distribution.id}" class="distribution"/><label for="distributionCheckBox_${distributionStatus.index}">${distribution.name}</label><br>
				</c:forEach>
				<input type="button" class="btn btn-primary" onclick="$('.distribution').attr('checked','true')" value="全选"/>
				<input type="button" class="btn btn-primary" onclick="$('.distribution').each(function(){this.checked = !this.checked;})" value="反选"/>
			</div>
		</div>
		<div class="control-group">
