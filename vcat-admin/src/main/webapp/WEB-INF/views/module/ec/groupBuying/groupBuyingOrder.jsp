...
<head>
    ...
    <script type="text/javascript">
            ...
            $("#btnExport").click(function(){
                top.$.jBox.confirm("确认导出？","系统提示",function(v){
                    if(v=="ok"){
                        ...
                        $("#searchForm").submit();
                        $("#searchForm").attr("action",ctx + "/ec/groupBuying/groupBuyingOrder");
                    }
                });
            });
        ...
        function reload(){
            reloadGrid('/ec/groupBuying/groupBuyingOrderData');
        }
        function clearForm(){
            $('#orderType').val("");
            $('#orderType').select2();
...
<form:form id="searchForm" modelAttribute="order" action="${ctx}/ec/groupBuying/groupBuyingOrder" method="get" class="breadcrumb form-search">
    ...
    <input id="groupBuyingId" name="sqlMap['groupBuyingId']" type="hidden" value="${order.sqlMap.groupBuyingId}">
    <input id="groupBuyingName" name="sqlMap['groupBuyingName']" type="hidden" value="${order.sqlMap.groupBuyingName}">
    <input id="skipGroupBuying" name="sqlMap['skipGroupBuying']" type="hidden" value="${order.sqlMap.skipGroupBuying}">
    <label>团购：${order.sqlMap.groupBuyingName}</label>
    <label>订单类型：</label>
