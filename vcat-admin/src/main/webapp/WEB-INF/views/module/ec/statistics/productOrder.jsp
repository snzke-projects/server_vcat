            ...
            $("#btnExport").click(function(){
                top.$.jBox.confirm("确认导出？","系统提示",function(v){
                    if(v=="ok"){
                        ...
                        $("#searchForm").submit();
                        $("#searchForm").attr("action",ctx + "/ec/statistics/productOrder");
                    }
                });
            });
...
<form:form id="searchForm" modelAttribute="order" action="${ctx}/ec/statistics/productOrderData" method="get" class="breadcrumb form-search">
    ...
    <input id="productId" name="sqlMap['productId']" type="hidden" value="${order.sqlMap.productId}">
    <input id="productName" name="sqlMap['productName']" type="hidden" value="${order.sqlMap.productName}">
    <input id="buyerId" name="sqlMap['buyerId']" type="hidden" value="${order.sqlMap.buyerId}">
    <input id="buyerName" name="sqlMap['buyerName']" type="hidden" value="${order.sqlMap.buyerName}">
    ...
    <input id="brandId" name="sqlMap['brandId']" type="hidden" value="${order.sqlMap.brandId}">
    <input id="supplierId" name="sqlMap['supplierId']" type="hidden" value="${order.sqlMap.supplierId}">
    <label>供应商：</label>
    <form:select id="supplierId" path="sqlMap['supplierId']" class="input-medium" disabled="true">
        <form:option value="" label="全部"/>
        <form:options items="${supplierList}" itemLabel="name" itemValue="id" htmlEscape="false"/>
    </form:select>
    <label>商品：${order.sqlMap.productName}</label>
    <c:if test="${null != order.sqlMap.buyerId && '' ne order.sqlMap.buyerId}"><label>买家：${order.sqlMap.buyerName}</label></c:if>
    <c:if test="${null != order.sqlMap.shopId && '' ne order.sqlMap.shopId}"><label>店铺：${order.sqlMap.shopName}</label></c:if>
    <label>订单类型：</label>
    ...
    <label>配送方：</label>
    <form:select id="distributionId" path="sqlMap['distributionId']" class="input-medium">
        <form:option value="" label="全部"/>
