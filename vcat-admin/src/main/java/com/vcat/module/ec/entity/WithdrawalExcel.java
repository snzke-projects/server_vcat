...
public class WithdrawalExcel extends DataEntity<WithdrawalExcel> {
    ...
    public String getStatusLabel() {
        if(Withdrawal.WITHDRAWAL_STATUS_UNTREATED == status){
            return "未处理";
        }else if(Withdrawal.WITHDRAWAL_STATUS_SUCCESS == status){
            return "提现完成";
        }else if(Withdrawal.WITHDRAWAL_STATUS_FAILURE == status){
            return "提现失败";
        }else{
            return "未知状态";
        }
