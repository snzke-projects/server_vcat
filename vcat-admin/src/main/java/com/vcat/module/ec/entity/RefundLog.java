...
public class RefundLog extends DataEntity<RefundLog> {
    ...
    public static RefundLog create(Refund refund){
        ...
        log.setStatusNote(createStatusNote(refund.getReturnStatus(),refund.getRefundStatus(),refund.getIsReceipt(),refund.getNote()));
        return log;
    }

    private static String createStatusNote(String returnStatus,String refundStatus,Integer isReceipt,String note){
        ...
        if(Refund.RETURN_STATUS_CHECK_SUCC.equals(returnStatus) && Refund.REFUND_STATUS_NO_REFUND.equals(refundStatus)){
            return "退货审核成功，可以退货";
        }else if(Refund.RETURN_STATUS_COMPLETED.equals(returnStatus) && Refund.REFUND_STATUS_NO_REFUND.equals(refundStatus) && null != isReceipt && isReceipt == 1){
            return "已成功收到退货物品，正在退款审核中...";
        }else if(Refund.RETURN_STATUS_COMPLETED.equals(returnStatus) && Refund.REFUND_STATUS_NO_REFUND.equals(refundStatus) && null != isReceipt && isReceipt == 0){
            return "退货完成，正在退款审核中...";
