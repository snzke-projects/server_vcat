...
public class Page<T> implements Serializable {
    ...
    private long countLimit = -1;// 总记录限制
    private int tempPageSize = getPageSize();// 当总记录限制小于页面大小时使用此参数缓存pageSize
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
        ...
        if (StringUtils.isNotBlank(countLimit) && StringUtils.isNumeric(countLimit)){
            setCountLimit(Long.parseLong(countLimit));
        }
	}
	...
	public void initialize(){
        if(countLimit > -1){
            if(countLimit <= getPageSize() && countLimit < getCount()){
                setCount(countLimit);
            }
...
//	}
    ...
    public void setCountLimit(long countLimit) {
        ...
        if(countLimit > -1 && countLimit < getPageSize()){
            setPageSize(new Long(countLimit).intValue());
        }
    }
