...
public class RefundBusiness {
    ...
    public RefundBusiness(Configure configure) throws IllegalAccessException, ClassNotFoundException, InstantiationException, UnrecoverableKeyException, IOException, NoSuchAlgorithmException, KeyStoreException, KeyManagementException {
    ...
    public interface ResultListener{
        ...
        void onFailByReturnCodeError(RefundReqData refundReqData, RefundResData refundResData);

        //API返回ReturnCode为FAIL，支付API系统返回失败，请检测Post给API的数据是否规范合法
        void onFailByReturnCodeFail(RefundReqData refundReqData, RefundResData refundResData);

        //支付请求API返回的数据签名验证失败，有可能数据被篡改了
        void onFailBySignInvalid(RefundReqData refundReqData, RefundResData refundResData);

        //退款失败
        void onRefundFail(RefundReqData refundReqData, RefundResData refundResData);

        //退款成功
        void onRefundSuccess(RefundReqData refundReqData, RefundResData refundResData);
    ...
    public void run(RefundReqData refundReqData,ResultListener resultListener) throws Exception {
        ...
        ReportReqData reportReqData = new ReportReqData(
                ...
                Configure.getIP(),configure
        );

        long timeAfterReport;
        if(Configure.isUseThreadToDoReport()){
            ReporterFactory.getReporter(reportReqData,configure).run();
            timeAfterReport = System.currentTimeMillis();
            Util.log("pay+report总耗时（异步方式上报）："+(timeAfterReport-costTimeStart) + "ms");
        }else{
            ReportService.request(reportReqData,configure);
            timeAfterReport = System.currentTimeMillis();
        ...
        if (refundResData == null || refundResData.getReturn_code() == null) {
            ...
            resultListener.onFailByReturnCodeError(refundReqData, refundResData);
            return;
        }
        ...
        if (refundResData.getReturn_code().equals("FAIL")) {
            ...
            resultListener.onFailByReturnCodeFail(refundReqData, refundResData);
        } else {
            log.i("退款API系统成功返回数据");
            //--------------------------------------------------------------------
            ...
            if (!Signature.checkIsSignValidFromResponseString(refundServiceResponseString,configure)) {
                setResult("Case3:退款请求API返回的数据签名验证失败，有可能数据被篡改了",Log.LOG_TYPE_ERROR);
                resultListener.onFailBySignInvalid(refundReqData, refundResData);
                return;
            }
            ...
            if (refundResData.getResult_code().equals("FAIL")) {
                ...
                resultListener.onRefundFail(refundReqData, refundResData);
            } else {
                //退款成功
                setResult("Case5:【退款成功】",Log.LOG_TYPE_INFO);
                resultListener.onRefundSuccess(refundReqData, refundResData);
            }
        }
    }
